...
consteval uint32_t hash(::std::string_view input)
{
    uint32_t result = 0xff'ff'ff'ff;
    for (auto character : input)
    {
        result = (result >> 8) ^ crc32[(result ^ character) & 0xff];
    }
 ...
 * @endcode
 */
#define essm_event_traits(name_) __essm_event_create_traits(::essm::detail::hash(#name_), name_)
